#Модуль, содержащий описание формы с вариацией коэффициента водного эквивалента снега для леса
...
def parseEntry(elem, message): #преобразование строки в вещественное число
    try:
        ...
        #замена запятой на точку и преобразование к вещественному числу
        return res
...
def run(): #обработчик для кнопки "Расчет"
    ...
    wEqField = parseEntry(snowFieldCoefEntry, "В качестве водного эквивалента снега для поля введено не число")
    wEqForestMin = parseEntry(snowForestLowBorder, 
        "В качестве нижней границы водного эквивалента снега для леса введено не число")
    wEqForestMax = parseEntry(snowForestHighBorder,
        "В качестве верхней границы водного эквивалента снега для леса введено не число")
    step = parseEntry(snowForestStep, "В качестве шага введено не число")
    if math.isnan(heightTh) or math.isnan(wEqField) or math.isnan(wEqForestMin) or math.isnan(wEqForestMax) \
        or math.isnan(step): return
    
    if (step > 0 and wEqForestMin > wEqForestMax) or (step < 0 and wEqForestMin < wEqForestMax):
    ...
    if not os.path.exists(app_folder + "/results_forest"): os.mkdir(app_folder + "/results_forest")

    wEqForest = wEqForestMin
    while wEqForest < wEqForestMax:
        #запуск расчетов
        data = fdc.FormData(pathLbl.cget("text"), startTime, endTime, heightTh, heightChk_state.get(),
            expChk_state.get(), wEqField, wEqForest, forestCoef, fieldCoef, sCoef, nCoef, wCoef, eCoef, pCoef)
        try:
            sml.run(data)
            #showinfo(title = "Данные успешно сохранены", message="Вы можете просмотреть файлы")
            if not os.path.exists(app_folder + "/results_forest/" + str(wEqForest).replace(".", ",")):
                os.mkdir(app_folder + "/results_forest/" + str(wEqForest).replace(".", ","))
            
            folder, name = os.path.split(data.file)
            os.replace(folder + "/les_group.dbf", app_folder + "/results_forest/" + str(wEqForest).replace(".", ",") + "/les_group.dbf")
            os.replace(folder + "/pole_group.dbf", app_folder + "/results_forest/" + str(wEqForest).replace(".", ",") + "/pole_group.dbf")

        except Exception as e:
            showinfo(title = "Ошибка при расчетах", message = str(e))
        finally: wEqForest += step
...
snowForestLbl = tk.Label(form, text = "Границы изменения водного эквивалента снега для леса")
snowForestLbl.grid(column = 0, row = 10, sticky = "W")
snowForestLowBorder = tk.Entry(form, width = 10)
snowForestLowBorder.grid(column = 1, row = 10)
snowForestHighBorder = tk.Entry(form, width = 10)
snowForestHighBorder.grid(column = 2, row = 10)
snowForestStepLbl = tk.Label(form, text = "Шаг")
snowForestStepLbl.grid(column = 3, row = 10)
snowForestStep = tk.Entry(form, width = 10)
snowForestStep.grid(column = 4, row = 10)
